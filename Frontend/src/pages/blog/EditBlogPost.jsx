...
const EditBlogPost = () => {
  ...
  const changeToCensoredText = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:7070/api/blogpost/draft/validate",
        {
          method: "POST",
          headers: {
      ...
      if (data.title != title || data.content != content) {
        ...
        setError(
          "Blog post contains inappropriate content. Please edit it. All inappropriate content will be replaced with '#'."
        );
        return;
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    const endpoint =
      submitType === "draft"
        ? "http://localhost:7070/api/blogpost/draft/update/" + draft.id
        : "http://localhost:7070/api/blogpost/draft/publish/" + draft.id;
